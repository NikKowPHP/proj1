...
export const StandardizationService = {
  standardize: (answers: Record<string, any>, units: "metric" | "imperial" = "metric"): Record<string, any> => {
    ...
    try {
      ...
      let validHeight = Number(answers.height_cm) || undefined;
      let validWeight = Number(answers.weight_kg) || undefined;

      if (units === 'imperial') {
        if (validHeight) validHeight = validHeight * INCHES_TO_CM;
      ...
      if (answers.occupational_hazards) {
        ...
        standardized.advanced.occupational = occupationalHistory.map((entry: any) => {
          return {
            ...
            isco: jobTitlesMap[entry.main_job_title?.toLowerCase()] || undefined,
      ...
      envKeys.forEach(key => {
        if (answers[key]) {
          if (key === 'env.indoor.solidfuel_years') {
            environmental[key] = Number(answers[key]);
          } else {
