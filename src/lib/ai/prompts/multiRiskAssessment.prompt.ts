...
export const getMultiRiskAssessmentPrompt = (
  ...
  const languageInstruction =
    locale === "pl"
      ? "Your response, including all text in 'overallSummary', 'explanation', 'factor', and 'recommendations' fields, MUST be in Polish."
...
**GUIDELINES:**
1.  **Language:** Strictly adhere to the language instruction provided above. All user-facing strings must be in the specified language.
2.  **Synthesized Summary is Key:** The 'overallSummary' is the most important part. It should connect the dots between different models. For instance, if 'smoking' impacts multiple models, mention that.
3.  **Group by Model:** The 'modelAssessments' array must group risk factors by their source model from the input.
4.  **Safety First:** Do NOT diagnose. Use phrases like "This assessment suggests...", "Factors like these are associated with...".
5.  **Map Risk Levels:** Convert the input level 'Average' to 'Moderate'. 'Low' and 'High' are the same.
6.  **Sensitive Topics:**
...
7.  **Crucial Disclaimer:** The final recommendation MUST ALWAYS be to consult a healthcare provider.
