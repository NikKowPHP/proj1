                        ...
                        {q.type === 'select' && (
                          <>
                            <Select onValueChange={(value) => onAnswer(q.id, value)} value={answers[q.id] || ""}>
                              <SelectTrigger id={q.id} className={error || externalErrors?.[q.id] ? "border-destructive" : ""}><SelectValue placeholder="Select an option" /></SelectTrigger>
                              <SelectContent>
                            ...
                            {(error || externalErrors?.[q.id]) && <p className="text-sm text-destructive">{error || externalErrors?.[q.id]}</p>}
                          </>
                        )}
                        {q.type === 'year_input' && (
                          <>
                            <YearInput
                              ...
                              aria-invalid={!!error || !!externalErrors?.[q.id]}
                            />
                            {(error || externalErrors?.[q.id]) && <p className="text-sm text-destructive">{error || externalErrors?.[q.id]}</p>}
                          </>
                        )}
